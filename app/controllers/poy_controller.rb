class PoyController < ApplicationController
  ...
  def download_supramap_kml
    @poyjob = PoyJob.first(:conditions => ["query_id = ?",params[:id]])
      send_data @poyjob.kml, :filename => "#{Study.find(@poyjob.query_id).name}.kml", :type => "kml"
  end

  def download_output
    @poyjob = PoyJob.first(:conditions => ["query_id = ?",params[:id]])
     send_data @poyjob.output, :filename => "#{Study.find(@poyjob.query_id).name}_output.txt" , :type => "txt"
  end

  def download_aligned_fasta
    @poyjob = PoyJob.first(:conditions => ["query_id = ?",params[:id]])
    send_data @poyjob.aligned_fasta, :filename => "#{Study.find(@poyjob.query_id).name}.ia", :type => "ia"
  end

  def download_tree
    @poyjob = PoyJob.first(:conditions => ["query_id = ?",params[:id]])
    send_data @poyjob.tree, :filename => "#{Study.find(@poyjob.query_id).name}.tre", :type => "tre"
  end
end
